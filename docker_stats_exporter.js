...
const argOptions = commandLineArgs([
  {
    ...
    alias: 'p',
    type: Number,
    defaultValue: process.env.DOCKERSTATS_PORT || 9487,
  },
  {
    name: 'interval',
    alias: 'i',
    type: Number,
    defaultValue: process.env.DOCKERSTATS_INTERVAL || 15,
  },
  {
    name: 'hostip',
    type: String,
    defaultValue: process.env.DOCKERSTATS_HOSTIP || '',
  },
  {
    name: 'hostport',
    type: Number,
  ...
  try {
      ...
      if (container.Id) {
        promises.push(
          docker
            ...
            .stats({ stream: false, decode: true }),
        );
      }
    }
    ...
    for (const result of results) {
      ...
      if (result.blkio_stats) {
        ...
        let ioWrite = 0.0;
        if (
          result.blkio_stats.io_service_bytes_recursive &&
          Array.isArray(result.blkio_stats.io_service_bytes_recursive)
        ) {
          for (const io of result.blkio_stats.io_service_bytes_recursive) {
